...
// Auto-create tables if they don't exist (safe for concurrent workers)
try {
  sqlite.exec(`
  ...
  // Table creation may fail if another worker is creating tables concurrently
  // Tables will already exist from the other worker, so this is safe to ignore
}
